...
namespace RealRoad
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // --- BEZIERPATCH ---
            // --- 
            Vector3 patchPosition = Vector3.Zero;

            Vector3[,] controlPoints = new Vector3[4, 4]
            {
                { new Vector3(-5, 0, -5), new Vector3(-5, 0, -1.666f), new Vector3(-5, 0, 1.666f), new Vector3(-5, 0, 5) },
                { new Vector3(-1.666f, 0, -5), new Vector3(-1.666f, 0, -1.666f), new Vector3(-1.666f, 0, 1.666f), new Vector3(-1.666f, 0, 5) },
                { new Vector3(1.666f, 0, -5), new Vector3(1.666f, 0, -1.666f), new Vector3(1.666f, 0, 1.666f), new Vector3(1.666f, 0, 5) },
                { new Vector3(5, 0, -5), new Vector3(5, 0, -1.666f), new Vector3(5, 0, 1.666f), new Vector3(5, 0, 5) }
            };

            var tesselation = new Tesselation(8, 8);

            var bezierPatch = BezierPatch.Add(map, patchPosition, controlPoints, tesselation);

            bezierPatch.QuadData = new TerrainQuadData
            {
                BrushMaterials = new List<Material> { new Material("34") } // "grass_ger_main"
            };

            bezierPatch.ViewDistance = 400;

            bezierPatch.Collision = true;

            bezierPatch.DetailVegetation = true;
            bezierPatch.VegetationSpheres = new List<VegetationSphere>();
            // -----------------------------------------------
            // -----------------------------------------------
            // -----------------------------------------------
